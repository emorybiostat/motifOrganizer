...
int readSeq(vector <motifClass> &fullMotifVector, const char outPutFileName[],const int INITIAL_CLUSTER, int &widthmin);
...
double chain(const int NUM_CYCLE, const int W, const int WMIN, const int WMAX, const int MAX_SHIFT, 
			 const short bin, const double FACTOR_0, list <clustClass> &bestClusters);
...
void resultDisplay(const int motifCount, const double loglike, const short bin, 
		   const double factor_0, list <clustClass> clusters, 
		    const char inputFileName[], const char outPutFileName[], const char motifFileName[]);
...
// read in data, initial assign clusters and calculate cluster profiles
	...
	WMIN = W;
	WMAX = W;
	initialCluster(bin,motifCount, fullMotifVector,initialclusternum);
	//exit(0);
// clustering
//-	cout << "cluster size= " << allClusts.size();
	...
	allClusts.clear();
// output result
	resultDisplay(motifCount, loglike, bin, factor_0, finalClusters,inputFileName, outputFileName,motifFileName);
	finalClusters.clear();
	return(0);
}//end of main
...
/*
    for (j=finalClustNumber;j>0;j--)
    {
		...
		for(motifsIDIter=(*sorted[j-1]).motifsID.begin();motifsIDIter !=(*sorted[j-1]).motifsID.end();motifsIDIter++)
		{
			...
			{
				motifblockFile << "> "<<inputFileName<<"cluster" <<finalClustNumber-j<<" "<< allMotifs[(*motifsIDIter)].name<<"  cluster "<<finalClustNumber-j+1<<endl;
...
*/
        ...
        for(clustIter = clusters.begin();clustIter != clusters.end();clustIter ++)
        {
                outPutFile << "cluster " << count << ", size= "<<(*clustIter).motifsID.size()<<"   ";// <<"Bayes Ratio=";
                outPutFile << "width = "<<(*clustIter).width;
                count++;
                value=finalRatioSingle(clustIter);
         	outPutFile << " Average Bayes ratio = " << value/(double) (*clustIter).motifsID.size() <<endl;
                tot=0;
                ...
                for(motifsIDIter=(*clustIter).motifsID.begin();motifsIDIter !=(*clustIter).motifsID.end();motifsIDIter++)
                {
                        ...
                        {  
                                motifblockFile << "> "<<inputFileName<<"cluster" <<finalClustNumber-j<<" "<< allMotifs[(*motifsIDIter)].name<<"  cluster "<<finalClustNumber-j+1<<endl;
                                for(m=0;m<allMotifs[(*motifsIDIter)].width;m++)
                                        motifblockFile <<allMotifs[(*motifsIDIter)].basepair[k][m+start];
                                motifblockFile <<endl;
                        }//end of k
                        //motifblockFile <<endl;
		 ...
		 const int WMAX, const int MAX_SHIFT, const double factor_0, list <clustClass> &finalClusters)
{
	...
	{
		cout << "chain" << j+1 << endl;
		like=chain(numItera, W, WMIN,WMAX, MAX_SHIFT, bin, factor_0, bestClusters);
...
int readSeq(vector <motifClass> &fullMotifVector, const char inputFileName[],const int INITIAL_CLUSTER, int &widthmin)
{
	int j;
