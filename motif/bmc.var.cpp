...
int readSeq(vector <motifClass> &fullMotifVector, const char outPutFileName[],int &widthmin);

void initialCluster(const short bin, const int motifCount, const vector <motifClass> fullMotifVector,const int INITIAL_SIZE);

//void initialCluster(const short bin, const int motifCount, const vector <motifClass> fullMotifVector,const int INITIAL_CLUSTER);
...
void resultDisplay(const int motifCount, const double loglike, const short bin, 
		   const double factor_0, list <clustClass> clusters, const char outPutFileName[], const char motifFileName[]);
...
int main(int argc, char **argv)
{
	...
	double loglike;
//	int IN_WIDTH;
	char inputFileName[MAX_LENGTH]="probfile2.txt";
...
//	srand((unsigned)time(NULL));
	...
	numIter = atoi(argv[5]);
	initialclustersize = atoi(argv[6]);
// read in data, initial assign clusters and calculate cluster profiles
	motifCount=readSeq(fullMotifVector,inputFileName,widthmin);
	W = widthmin;
	WMIN = W;
	WMAX = W;
	initialCluster(bin,motifCount, fullMotifVector,initialclustersize);
	//exit(0);
// clustering
//-	cout << "cluster size= " << allClusts.size();
	loglike = bmc(bin,numChains,numIter, W, WMIN,WMAX,MAX_SHIFT,factor_0,finalClusters);
// output result
	resultDisplay(motifCount, loglike, bin, factor_0, finalClusters,outputFileName,motifFileName);
	return(0);
}//end of main
		   ...
		   const double factor_0, list <clustClass> clusters, const char outPutFileName[], const char motifFileName[])
{
	...
	motifblockFile.open(motifFileName);
	if(!motifblockFile)
	{
...
*/
	...
	sum = 0;
        ...
        {
                sum = sum + allMotifs[(*motifsIDIter)].geneSize;
        }  
...
//***** delete 02/18/06
		...
		outPutFile << " total number of motifs = " <<sum<<endl;
		tot=0;
		for(motifsIDIter=(*sorted[j-1]).motifsID.begin();motifsIDIter !=(*sorted[j-1]).motifsID.end();motifsIDIter++)
		{
			tot++;
		...
		for(motifsIDIter=(*sorted[j-1]).motifsID.begin();motifsIDIter !=(*sorted[j-1]).motifsID.end();motifsIDIter++)
		{
			total = allMotifs[(*motifsIDIter)].geneName.size();
			for(k=0;k<total;k++)
				outPutFile << allMotifs[(*motifsIDIter)].geneName[k]<<endl;	
...
// if the current cluster is the best, then do nothing
	...
	{// same cluster
		...
		clustlong.clear();
		if((clustWidth==motifWidth)&&(oldmotifshift == allMotifs[motifOrder].motifshift)&&(oldclustshift == allMotifs[motifOrder].clustshift))
		{
...
int readSeq(vector <motifClass> &fullMotifVector, const char inputFileName[], int &widthmin)
{
	int j;
...
//			temMotif.cluster = motifCount % INITIAL_CLUSTER;
		...
		else if(c=='>') // first line of FASTA format, take name
		{
			...
			if((signal==0)||(tempName!=temMotif.name))
			{
				...
				rowCount=0;
				iss >> geneName;
				temMotif.geneName.push_back(geneName);
				iss >> geneSize;
				cout <<geneSize<<endl;
				temMotif.geneSize = geneSize;
				iss.ignore();
			}
			else // new 02/23/06 subsequent lines of FASTA annotation, take name and size
			{	
				iss >> newGeneName;
				if(geneName !=newGeneName)
				{
					geneName = newGeneName;
					 temMotif.geneName.push_back(newGeneName);
				}
			}
		}
