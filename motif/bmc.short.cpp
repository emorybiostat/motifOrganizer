...
//***** delete 02/18/06
		...
		for(motifsIDIter=(*sorted[j-1]).motifsID.begin();motifsIDIter !=(*sorted[j-1]).motifsID.end();motifsIDIter++)
		{
			...
			{
				motifblockFile << "> "<<inputFileName<<"cluster" <<finalClustNumber-j+1<<" "<< allMotifs[(*motifsIDIter)].name <<" " << (*sorted[j-1]).size<<endl;
				for(m=0;m<(*sorted[j-1]).width;m++)
					motifblockFile <<allMotifs[(*motifsIDIter)].basepair[k][m];
				motifblockFile <<endl;
			}//end of k
		}//end of motifsIDIter
...
//227			like1=loglikelihoodSingle(stage);
			like=like+difference;
//218			cout << "like=" <<like << "like1=" <<like1<<endl;
//227			if (fabs(like-like1)>0.00001)
//227			{
//227				cout << "problem here: motif = "<< j<<" dif=" << difference << "new like=" << like << " like1=" <<like1<<endl;
//227				exit(18);
//227			}
//***** special check on clusters
//			for(allClustsIter = allClusts.begin();allClustsIter != allClusts.end(); allClustsIter++)
